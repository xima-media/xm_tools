        ...
        if ($this->isImageRegenerationRequired($image)) {
            ...
            foreach ($sizes['width'] as $key => $width) {
                ...
                if ($typo3Version >= 8006000) {
                    ...
                    $processingInstructions['crop'] = $cropArea->isEmpty() ? null : $cropArea->makeAbsoluteBasedOnFile(
                        $image
                    );

                    if ($mode === 'c' && !$focusArea->isEmpty()) {
                        $processingInstructions = $this->applyCropShiftingInstructionsBasedOnFocusArea(
